:: Make sure to run this script with the XDK shader compiler located at
::  C:\Program Files (x86)\Microsoft Durango XDK\[release]\xdk\FXC\amd64

@echo off

if exist Shaders.tar del Shaders.tar
if exist Shaders.xbox.h del Shaders.xbox.h
mkdir pdb
...
bin2h.py Shaders.z Shaders >> Shaders.xbox.h
...
:: -------------------------------------------------------------------------------------------------------
:fxc_vs
    :: set debug_options=/Zi /Qstrip_debug /Fd pdb/%1.pdb
    fxc /T vs_4_0 /nologo /O3 %debug_options%  /Qstrip_reflect -D__XBOX_DISABLE_SHADER_NAME_EMPLACEMENT=1 -D__XBOX_FULL_PRECOMPILE_PROMISE=1 -D__XBOX_DISABLE_PRECOMPILE_LS=1 -D__XBOX_DISABLE_PRECOMPILE_ES=1 /Fo %1.o %2 %3 %4 %5 %6 > NUL
    if %errorlevel% neq 0 EXIT /B %errorlevel%
    tar.py Shaders.tar %1.o >> Shaders.xbox.h
    del %1.o
    EXIT /B 0

:fxc_ps
    :: set debug_options=/Zi /Qstrip_debug /Fd pdb/%1.pdb
    fxc /T ps_4_0 /nologo /O3 %debug_options% /Qstrip_reflect -D__XBOX_DISABLE_SHADER_NAME_EMPLACEMENT=1 -D__XBOX_FULL_PRECOMPILE_PROMISE=1 /Fo %1.o %2 %3 %4 %5 %6 > NUL
    if %errorlevel% neq 0 EXIT /B %errorlevel%
    tar.py Shaders.tar %1.o >> Shaders.xbox.h
    del %1.o
    EXIT /B 0
...
struct MouseControlledCamera : public Engine::Component<MouseControlledCamera> {
    ...
    {};
    MouseControlledCamera(const BindingProperty& data);
...
using CubeWorld::MouseControlledCamera;

template<>
inline auto registerMembers<MouseControlledCamera>()
{
    return members(
        member("pitch_sensitivity", &MouseControlledCamera::pitchSensitivity),
        member("yaw_sensitivity", &MouseControlledCamera::yawSensitivity)
    );
}

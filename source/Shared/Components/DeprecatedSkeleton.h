...
public:
   ...
   DeprecatedSkeleton();
   DeprecatedSkeleton(const std::string& file) { Load(file); }
   DeprecatedSkeleton(Engine::ComponentHandle<VoxModel> model) : model(model) {}
   DeprecatedSkeleton(const std::string& file, Engine::ComponentHandle<VoxModel> model) : model(model) { Load(file); }
   void Reset();

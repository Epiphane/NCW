...
public:
    Worker(noise::module::Perlin& heightmodule)
    {
        mShared.heightmodule = &heightmodule;
    ...
    void Cancel()
    {
        std::unique_lock<std::mutex> lock{mShared.requestsMutex};
    ...
    void BuildChunk(const Request& request)
    {
        ...
        {
            std::unique_lock<std::mutex> lock{mShared.moduleMutex};
            noise::utils::NoiseMapBuilderPlane builder;
            builder.SetSourceModule(*mShared.heightmodule);
        ...
        for (uint32_t x = 0; x < kChunkSize; ++x)
        {
            for (uint32_t z = 0; z < kChunkSize; ++z)
            {
                ...
                else if (elevation >= -0.125f) { source = SAND; dest = GRASS; start = 0.0625f; end = 0.125f; }
                else if (elevation >= -0.3f) { source = SHORE; dest = SAND; start = 0; end = 0.0625f; }
                else if (elevation >= -0.65f) { source = SHALLOW; dest = SHORE; start = -0.25f; end = 0; }
...
{
    mWorker.reset(new Worker(mHeightmodule));
}

...
class UIStackView : public UIElementDep
{
public:
   //
   ...
   UIStackView(UIRootDep *root, UIElementDep *parent, const std::string& name = "");

   virtual UIElementDep *AddChild(std::unique_ptr<UIElementDep> &&element) override;
...
private:
   ...
   void CreateChildConstraints(const std::vector<UIElementDep*>& activeChildren);

   void RemakeConstraints();

   // Helper function that creates constraints to honor SetAlignItemsBy
   void CreateConstraintsForItemAlignment(const std::vector<UIElementDep*>& activeChildren);

...
class UIRootDep : public UIElementDep, public EventManager
{

friend class UIRoot_ConstraintDebugging;

public:
   UIRootDep(Input* input);
   ~UIRootDep();
   ...
   void RemoveConstraintsForElement(const UIElementDep* element);
   ...
   //    each axis
   //
   void DebugLogConstraintsForElement(const UIElementDep* element);
   ...
   virtual void ElementDestructing(UIElementDep* element);
...
public:
   ...
   UIElementDep* AddChild(std::unique_ptr<UIElementDep> &&element) override;

private:
   // Helper function that gives a shallow copy of just the active elements that contain the given point
   void GetActiveElementsContainingPoint(const std::vector<UIElementDep*> &elementList, double pointX, double pointY, std::vector<UIElementDep*>* outElementList);
   ...
   std::vector<UIElementDep*> mElements;
   ...
   UIElementDep* mActivelyCapturingElement;

   // UIElementDep that will capture mouse events if active and show you constraint information.
